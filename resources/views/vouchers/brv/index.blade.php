                                ...
                                <div class="page-title-box">
                                    <div class="btn-group float-right">
                                        <ol class="breadcrumb hide-phone p-0 m-0">
                                            ...
                                            <li class="breadcrumb-item active">List Of Bank Receipt Vouchers</li>
                                        </ol>
                                    </div>
                                    <h4 class="page-title">List Of Bank Receipt Vouchers</h4>
                                </div>
                            </div>
                        </div>
                        <div class="form-group button-items mb-0 text-right">
                            <a href="{{ route('brv.create') }}" class="btn btn-primary waves-effect waves-light">Create
                                Bank Receipt Voucher</a>
                        </div>
                        <br>
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="card m-b-30">
                                    <div class="card-body">
                                        <form action="{{ route('brv.list') }}" method="get" id="form-search">
                                            <div class="row">
                                                <div class="col-md-3">
                                                ...
                                                <div class="col-md-3">
                                                    <div class="form-group">
                                                        ...
                                                        <select name="order_by" id="order_by"
                                                            ...
                                                            <option value="brv_no" <?php if(isset($_GET["order_by"]) && $_GET["order_by"]=="brv_no") { echo " selected"; } ?> >Voucher No</option>
                                    ...
                                    <div class="card-body">
                                        @if($brvData)
                                            <table class="table table-hover subject-table">
                                                ...
                                                @foreach($brvData as $brv)
                                                    <tr id="row_{{$brv->id}}">
                                                        <td>{{ $brv->brv_no }}</td>
                                                        <td width="300">{{ $brv->project->name }}</td>
                                                        <td>{{ \Carbon\Carbon::parse($brv->date)->format('d-m-Y') }}</td>
                                                        <td>{{ $brv->bank->name }}</td>
                                                        <td>{{ $brv->paid_to }}</td>
                                                        <td>{{ number_format($brv->amount,2) }}</td>
                                                        <td>
                                                            @if(!empty($permission->edit_access) && $permission->edit_access == 1 || Auth::user()->is_admin == 1)
                                                                <a href="{{ route('brv.edit',['id'=>$brv->id]) }}" title="Edit"><i
